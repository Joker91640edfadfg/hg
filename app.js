...
const fs = require("fs");
const lozayar = JSON.parse(fs.readFileSync("./Jokerayar.json", { encoding: "utf-8" }));
...
Voice.on("ready", async() => {
    ...
    Voice.user.setPresence({
        status: "online",
        activity: {
            name: Jokerayar.DEFAULTS.ACTIVITY_TEXT
        }
    });

    Voice.log(`Jokerayar | \'${Voice.user.username}\' Bot Başlatıldı.!`);
    ...
    const Channel = Guild.channels.cache.get(Jokerayar.DEFAULTS.VOICE_CHANNEL);
    if(!Channel) {
